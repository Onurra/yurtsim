...
// Geçerli bir kayıt var mı? Hem menü ("devam et" butonu) hem "yeni oyun" guard'ı bunu kullanır;
// mantık renderMainMenu'deki tespitle birebir aynı tutuldu (tutarlılık için).
function hasSaveGame(){try{const raw=localStorage.getItem(SAVE_KEY);if(!raw)return false;const d=JSON.parse(raw);if(d&&d.playerName&&d.playerLastName)return true;return raw.length>20}catch(e){return false}}
// Eski / sürümsüz kayıtları güncel şemaya taşı (yerinde). saveVersion yoksa v1 kabul edilir.
// Ekstra sistem alanlarını (year/weather/notifs...) zaten ensureExtState dolduruyor; burası
// ileride şema değişirse gerçek migration adımlarının ekleneceği yer.
function migrateSave(data){if(!data)return data;const v=data.saveVersion||1;/* v1 -> v2: yalnız sürüm damgası, ek alanlar ensureExtState'te */data.saveVersion=SAVE_VERSION;return data}
function saveGame(){try{const data={...state};delete data.courses;data.saveVersion=SAVE_VERSION;localStorage.setItem(SAVE_KEY,JSON.stringify(data))}catch(e){console.warn('Kayıt başarısız:',e)}}
function loadGame(){try{const saved=localStorage.getItem(SAVE_KEY);if(!saved)return false;let loaded=JSON.parse(saved);if(!loaded||typeof loaded!=='object')throw new Error('bozuk kayıt');loaded=migrateSave(loaded);Object.keys(loaded).forEach(k=>{state[k]=loaded[k]});const expectedFriends=state.gender==='kız'?FRIENDS_KIZ:FRIENDS_ERKEK;if(!state.friends||!state.friends.length||!state.friends.find(f=>f.id===expectedFriends[0].id)){state.friends=JSON.parse(JSON.stringify(expectedFriends))}else{expectedFriends.forEach(df=>{if(!state.friends.find(f=>f.id===df.id))state.friends.push(df)})}if(state.gender==='kız'){const looksErkek=state.guzCourses&&state.guzCourses[0]&&!/^HUK|^ATA1/.test(state.guzCourses[0].code);if(looksErkek||!loaded.dates||!loaded.dates.find(d=>d.id==='demir')){state.guzCourses=state.guzCoursesKiz;state.baharCourses=state.baharCoursesKiz;state.dates=state.datesKiz}}state.courses=state.semester==='bahar'?state.baharCourses:state.guzCourses;state.activeModal=null;return true}catch(e){console.warn('Yükleme başarısız:',e);localStorage.removeItem(SAVE_KEY);return false}}
...
if(hasSaveGame()&&!confirm("Mevcut bir kaydın var. Yeni oyun başlatırsan bu ilerleme kalıcı olarak silinir. Devam edilsin mi?"))return;
