...
<div style="background:var(--surface);border:0.5px solid ${C.bt};border-radius:6px;padding:10px 12px;margin-bottom:12px;">
...
<div style="font-size:11px;color:${C.ts};margin-bottom:6px;"><i class="ti ti-calendar-event"></i> Buluşma</div><div style="background:var(--surface);border:0.5px solid ${C.bt};border-radius:6px;overflow:hidden;margin-bottom:14px;">`;
...
h+=`</div><div style="font-size:11px;color:${C.ts};margin-bottom:6px;"><i class="ti ti-gift"></i> Hediye al</div><div style="background:var(--surface);border:0.5px solid ${C.bt};border-radius:6px;overflow:hidden;margin-bottom:14px;">`;
...
h+=`</div><div style="font-size:11px;color:${C.ts};margin-bottom:6px;"><i class="ti ti-plane"></i> Yurt dışı tatil</div><div style="background:var(--surface);border:0.5px solid ${C.bt};border-radius:6px;overflow:hidden;margin-bottom:14px;">`;
...
<div style="background:var(--surface);border:0.5px solid ${C.bt};border-radius:8px;padding:14px;margin-bottom:14px;">
...
h+=`<div style="font-size:10.5px;color:${tierColors[t]};font-weight:700;margin-bottom:5px;text-transform:uppercase;letter-spacing:0.5px;">${t}</div><div style="background:var(--surface);border:0.5px solid ${C.bt};border-radius:6px;overflow:hidden;margin-bottom:12px;">`;
items.forEach((m,i)=>{
...
h+=`<div style="font-size:11px;color:${C.ts};margin-bottom:10px;line-height:1.4;">Yeni kıyafet moral artırır · ${wardrobe.length} parça gardırobunda · 45 dk geçer</div><div style="background:var(--surface);border:0.5px solid ${C.bt};border-radius:6px;overflow:hidden;">`;
items.forEach((c,i)=>{
...
// ===== Mesajlaşma (WhatsApp tarzı) =====
// state.chatOpen = açık kişinin id'si (null = sohbet listesi). Davet / imza / tuvalet
// kağıdı ilgili kişinin thread'ine gelen mesaj olarak düşer; aksiyonlar mevcut
// acceptInvite / declineInvite / openModal('signature') / orderToiletPaper'ı kullanır.
const CHAT_SIGNER_ID='_signer';
function isRoommateId(id){return id===(state.gender==='kız'?'sude':'evren')}
function chatUnread(id){
if(state.pendingInvite&&state.pendingInvite.fid===id)return true;
if(isRoommateId(id)&&state.toiletPaperPending&&!state.toiletPaperSnoozed)return true;
return false;
}
function buildChatContacts(){
const list=[];
const gf=typeof getGf==='function'?getGf():null;
if(gf)list.push({id:'gf',name:gf.name,initial:(gf.name||'?')[0],color:gf.color||'#D4537E',kind:'gf',tag:gf.dept||gf.tag});
(state.friends||[]).forEach(f=>list.push({id:f.id,name:f.name,initial:f.initial||(f.name||'?')[0],color:f.color||'#534AB7',kind:f.id==='anne'?'anne':'friend',tag:f.tag}));
list.push({id:CHAT_SIGNER_ID,name:signerName()+' Hoca',initial:signerInitial(),color:'#0F6E56',kind:'signer',tag:'Ders hocası'});
return list;
}
function chatUnreadCount(){return buildChatContacts().filter(c=>chatUnread(c.id)).length}
const chatFlavor={
mert:'Bu akşam maç var, geliyor musun?',
emre:'Hava güzel, sahile inelim mi bir ara',
kerem:'Memleketten selam! Ne zaman görüşüyoruz',
salih:'Pazar halısaha ayarlandı, sen de gel',
evren:'Kanka bulaşıklar birikti bu arada',
selin:'Ders notlarını attım, baksana',
leyla:'Cafede oturalım mı bugün, konuşacaklarım var',
umay:'Yeni bir sergi açılmış, gidelim mi',
yildiz:"Brunch'a çıkıyoruz, hazırlan",
eylul:'Sabah koşusuna kaldır beni',
sude:'Akşam dizi maratonu var, çayı koydum',
nazli:'Çalışma grubu bu akşam, geliyor musun'
};
function buildThread(c){
maybeRotateMessages();
const bubbles=[];const id=c.id;let action=null;
if(c.kind==='signer'){
bubbles.push({side:'in',text:'Derse devam durumun nasıl gidiyor?'});
bubbles.push({side:'in',text:state.msgRotation.S});
action='signer';
}else if(c.kind==='gf'){
const rel=state.relationship||0;
bubbles.push({side:'in',text:rel>=75?'Seni çok özledim 🥰 bugün buluşalım mı?':rel>=50?'Napıyorsun, müsait misin?':rel>=25?'Neredesin, günlerdir aramadın...':'Konuşmamız gereken şeyler var.'});
action='gf';
}else if(id==='anne'){
bubbles.push({side:'in',text:'Nasılsın, düzgün besleniyor musun?'});
bubbles.push({side:'in',text:state.msgRotation.A});
}else{
const f=(state.friends||[]).find(x=>x.id===id);const aff=f?f.affinity:50;
bubbles.push({side:'in',text:aff>=70?'Kankaaa naber, görüşmeyeli çok oldu!':aff>=40?'Naber, ne yapıyorsun?':'Selam.'});
if(chatFlavor[id])bubbles.push({side:'in',text:chatFlavor[id]});
if(state.pendingInvite&&state.pendingInvite.fid===id){bubbles.push({side:'in',text:state.pendingInvite.text});action='invite';}
else if(isRoommateId(id)&&state.toiletPaperPending&&!state.toiletPaperSnoozed){bubbles.push({side:'in',text:'Tuvalet kağıdı ve sabun sırası sende kanka'});action='toilet';}
}
const sent=(state.chats&&state.chats[id])||[];
sent.forEach(t=>bubbles.push({side:'out',text:t}));
return {bubbles,action};
}
function sendChat(id,text){if(!state.chats)state.chats={};if(!state.chats[id])state.chats[id]=[];state.chats[id].push(text);if(state.chats[id].length>6)state.chats[id]=state.chats[id].slice(-6);render();}
function openChat(id){state.chatOpen=id;render();}
function closeChat(){state.chatOpen=null;render();}
function chatAvatarHtml(c,size){const s=size||46;return `<div style="width:${s}px;height:${s}px;border-radius:50%;background:${c.color};color:white;font-size:${Math.round(s*0.4)}px;display:flex;align-items:center;justify-content:center;font-weight:700;flex-shrink:0;">${c.initial}</div>`;}
function modalMessagesHtml(){
maybeRotateMessages();
if(state.chatOpen){const c=buildChatContacts().find(x=>x.id===state.chatOpen);if(c)return chatThreadHtml(c);state.chatOpen=null;}
return chatListHtml();
}
...
contacts.forEach(c=>{
const inc=buildThread(c).bubbles.filter(b=>b.side==='in');
const raw=inc.length?inc[inc.length-1].text:'';const preview=raw.slice(0,38);
const un=chatUnread(c.id);
h+=`<div onclick="openChat('${c.id}')" style="display:flex;gap:11px;align-items:center;padding:10px 4px;border-bottom:0.5px solid ${C.bt};cursor:pointer;">${chatAvatarHtml(c,46)}<div style="flex:1;min-width:0;"><div style="display:flex;justify-content:space-between;align-items:center;gap:6px;"><span style="font-weight:700;font-size:14px;color:${C.tp};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${c.name}</span>${un?'<span style="width:9px;height:9px;border-radius:50%;background:#25D366;flex-shrink:0;"></span>':''}</div><div style="color:${un?'#1F7A46':C.ts};font-size:12px;font-weight:${un?'600':'400'};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;line-height:1.4;">${preview}${raw.length>38?'…':''}</div></div></div>`;
});
h+='</div>';
return h;
}
function chatThreadHtml(c){
const th=buildThread(c);
let bub='';
th.bubbles.forEach(b=>{
if(b.side==='in')bub+=`<div style="display:flex;justify-content:flex-start;margin-bottom:6px;"><div style="max-width:78%;background:var(--surface);border:0.5px solid ${C.bt};color:${C.tp};font-size:13px;line-height:1.4;padding:8px 11px;border-radius:14px 14px 14px 4px;">${b.text}</div></div>`;
else bub+=`<div style="display:flex;justify-content:flex-end;margin-bottom:6px;"><div style="max-width:78%;background:#0F6E56;color:white;font-size:13px;line-height:1.4;padding:8px 11px;border-radius:14px 14px 4px 14px;">${b.text}</div></div>`;
});
let actions='';
if(th.action==='invite')actions=`<div style="display:flex;gap:8px;"><button onclick="acceptInvite()" style="flex:1;font-size:13px;padding:10px;background:#1F4A11;color:white;border:none;border-radius:10px;font-weight:700;cursor:pointer;font-family:inherit;">✓ Kabul et</button><button onclick="declineInvite()" style="flex:1;font-size:13px;padding:10px;background:var(--surface);color:${C.tp};border:0.5px solid ${C.bt};border-radius:10px;font-weight:700;cursor:pointer;font-family:inherit;">Reddet</button></div>`;
else if(th.action==='signer')actions=`<button onclick="openModal('signature')" style="width:100%;font-size:13px;padding:10px;background:#0F6E56;color:white;border:none;border-radius:10px;font-weight:700;cursor:pointer;font-family:inherit;">✍️ İmza iste</button>`;
else if(th.action==='toilet')actions=`<div style="display:flex;gap:8px;"><button onclick="orderToiletPaper()" style="flex:1;font-size:13px;padding:10px;background:#1F4A11;color:white;border:none;border-radius:10px;font-weight:700;cursor:pointer;font-family:inherit;">Sipariş et · 300₺</button><button onclick="postponeToiletPaper()" style="flex:1;font-size:13px;padding:10px;background:var(--surface);color:${C.tp};border:0.5px solid ${C.bt};border-radius:10px;font-weight:700;cursor:pointer;font-family:inherit;">Ertele</button></div>`;
else if(th.action==='gf')actions=`<button onclick="openModal('girlfriend')" style="width:100%;font-size:13px;padding:10px;background:#D4537E;color:white;border:none;border-radius:10px;font-weight:700;cursor:pointer;font-family:inherit;">💕 Sevgili menüsü</button>`;
let chips='';
if(!th.action){const replies=['👍','Napıyon?','Sonra yazarım','Görüşürüz'];chips='<div style="display:flex;flex-wrap:wrap;gap:6px;">'+replies.map(r=>`<button onclick="sendChat('${c.id}','${r}')" style="font-size:12px;padding:6px 12px;background:var(--surface);color:${C.tp};border:0.5px solid ${C.bt};border-radius:16px;cursor:pointer;font-family:inherit;">${r}</button>`).join('')+'</div>';}
return `<div style="display:flex;align-items:center;gap:9px;padding-bottom:10px;margin-bottom:12px;border-bottom:0.5px solid ${C.bt};"><button onclick="closeChat()" style="background:none;border:none;font-size:22px;color:${C.tp};cursor:pointer;padding:0 2px;line-height:1;font-family:inherit;">‹</button>${chatAvatarHtml(c,38)}<div style="min-width:0;"><div style="font-weight:700;font-size:14.5px;color:${C.tp};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${c.name}</div>${c.tag?`<div style="font-size:10.5px;color:${C.ts};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${c.tag}</div>`:''}</div></div><div style="min-height:110px;margin-bottom:10px;">${bub}</div><div style="display:flex;flex-direction:column;gap:8px;">${actions}${chips}</div>`;
}
function renderMessages(){maybeRotateMessages();const el=document.getElementById('invites');if(!el)return;const showTp=state.toiletPaperPending&&!state.toiletPaperSnoozed;if(!state.pendingInvite&&!showTp){el.style.display='none';el.innerHTML='';return}el.style.display='block';let html='';if(state.pendingInvite){const i=state.pendingInvite;html+=`<div style="background:#FAEEDA;border:1px solid #EF9F27;border-radius:12px;padding:10px 12px;margin-bottom:10px;"><div style="display:flex;gap:9px;align-items:flex-start;"><div style="width:32px;height:32px;border-radius:50%;background:${i.color};color:white;font-size:13px;display:flex;align-items:center;justify-content:center;font-weight:700;flex-shrink:0;">${i.initial}</div><div style="flex:1;min-width:0;"><div style="font-weight:700;font-size:12.5px;color:${C.tp};">${i.from}</div><div style="color:${C.ts};font-size:11.5px;line-height:1.35;margin-bottom:7px;">"${i.text}"</div><div style="display:flex;gap:6px;"><button onclick="acceptInvite()" style="font-size:11px;padding:5px 13px;background:#1F4A11;color:white;border:none;border-radius:8px;font-weight:600;cursor:pointer;font-family:inherit;">Kabul</button><button onclick="declineInvite()" style="font-size:11px;padding:5px 13px;background:var(--surface);color:${C.tp};border:0.5px solid ${C.bt};border-radius:8px;font-weight:600;cursor:pointer;font-family:inherit;">Red</button></div></div></div></div>`}if(showTp){html+=`<div style="background:#EEEDFE;border:1px solid #6B61D9;border-radius:12px;padding:10px 12px;margin-bottom:10px;"><div style="display:flex;gap:9px;align-items:flex-start;"><div style="width:32px;height:32px;border-radius:50%;background:#1B3A5C;color:white;font-size:16px;display:flex;align-items:center;justify-content:center;font-weight:700;flex-shrink:0;">🧻</div><div style="flex:1;min-width:0;"><div style="font-weight:700;font-size:12.5px;color:${C.tp};">Oda arkadaşları</div><div style="color:${C.ts};font-size:11.5px;line-height:1.35;margin-bottom:7px;">"Tuvalet kağıdı ve sabun sırası sende kanka"</div><div style="display:flex;gap:6px;"><button onclick="orderToiletPaper()" style="font-size:11px;padding:5px 13px;background:#1F4A11;color:white;border:none;border-radius:8px;font-weight:600;cursor:pointer;font-family:inherit;">Sipariş et · 300₺</button><button onclick="postponeToiletPaper()" style="font-size:11px;padding:5px 13px;background:var(--surface);color:${C.tp};border:0.5px solid ${C.bt};border-radius:8px;font-weight:600;cursor:pointer;font-family:inherit;">Ertele</button></div></div></div></div>`}el.innerHTML=html}
