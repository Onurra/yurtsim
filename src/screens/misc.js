...
h+=`<div style="display:flex;gap:3px;margin-bottom:10px;background:white;padding:3px;border-radius:8px;border:0.5px solid ${C.bt};">`;
tabs.forEach(t=>{const active=tab===t.id;h+=`<button onclick="setFoodTab('${t.id}')" style="flex:1;padding:6px 4px;border:none;border-radius:5px;background:${active?'#F5F4EE':'transparent'};color:${active?C.tp:C.ts};font-size:10px;font-weight:600;cursor:pointer;font-family:inherit;display:flex;flex-direction:column;align-items:center;gap:2px;line-height:1.1;">
...
h+=`<div style="font-size:10.5px;color:#27500A;margin-bottom:6px;font-weight:600;">🏠 KYK yurt yemekhanesi · BEDAVA</div><div style="background:white;border:0.5px solid ${C.bt};border-radius:6px;overflow:hidden;">
...
h+=`<div style="font-size:10.5px;color:#0C447C;margin-bottom:6px;font-weight:600;">🎓 İTÜ kampüs yemekhanesi · ucuz</div><div style="background:white;border:0.5px solid ${C.bt};border-radius:6px;overflow:hidden;">
...
h+=`<div style="background:#FAEEDA;border:0.5px solid #EF9F27;border-radius:6px;padding:14px;text-align:center;font-size:11px;color:#854F0B;line-height:1.5;">🏠 Yurda veya 🎓 kampüse git de yemekhaneden ye.<br><span style="font-size:9.5px;color:#5F5E5A;">Yurt: bedava (06-12, 16-22) · Kampüs: 25-40₺ (07-21)</span></div>`;
}
}else if(tab==='yemeksele'){
ensureFoodDiscounts();
h+=`<div style="background:white;border:0.5px solid ${C.bt};border-radius:6px;overflow:hidden;">`;
...
h+=`<div style="background:#FAEEDA;border:0.5px solid #C2410C;border-radius:6px;padding:7px 10px;margin-bottom:6px;font-size:10px;color:#854F0B;line-height:1.4;">Tokluk geliyor ama moral düşer. Param yokken son çare.</div><div style="background:white;border:0.5px solid ${C.bt};border-radius:6px;overflow:hidden;">`;
...
ov.innerHTML=`<div onclick="event.stopPropagation()" style="background:white;border-radius:14px;padding:14px;width:248px;text-align:center;cursor:default;box-shadow:0 12px 32px rgba(0,0,0,0.4);">
...
<div style="font-size:16px;font-weight:700;color:#1F1F1D;">${d.name}, ${d.age}</div>
<div style="font-size:11px;color:#5F5E5A;margin-top:3px;">${d.dept}</div>
<div style="font-size:11px;color:#5F5E5A;margin-top:6px;font-style:italic;line-height:1.4;">"${d.tag}"</div>
<div style="display:flex;justify-content:space-between;margin-top:11px;font-size:10.5px;color:#5F5E5A;padding:7px 0;border-top:0.5px solid #D5D2C8;border-bottom:0.5px solid #D5D2C8;">
...
h+=`</div><div style="font-size:11px;color:${C.ts};margin-bottom:6px;"><i class="ti ti-device-gamepad-2"></i> Oyun · bedava</div><div style="background:white;border:0.5px solid ${C.bt};border-radius:6px;overflow:hidden;margin-bottom:14px;">`;
