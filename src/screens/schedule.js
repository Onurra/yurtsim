...
let h=`<div style="display:flex;gap:4px;margin-bottom:14px;padding:3px;background:var(--bg-app);border:0.5px solid ${C.bt};border-radius:8px;">
...
h+=`<div style="display:flex;gap:3px;margin-bottom:12px;background:var(--surface);padding:3px;border-radius:8px;border:0.5px solid ${C.bt};">`;
days.forEach(d=>{const active=tab===d;const today=state.dayName===d;
h+=`<button onclick="setScheduleTab('${d}')" style="flex:1;padding:7px 4px;border:none;border-radius:5px;background:${active?'var(--bg-app)':'transparent'};color:${active?C.tp:C.ts};font-size:10.5px;font-weight:600;cursor:pointer;font-family:inherit;position:relative;line-height:1.2;">${d}${today?'<span style="position:absolute;top:3px;right:5px;width:5px;height:5px;background:#E24B4A;border-radius:50%;"></span>':''}</button>`});
h+=`</div>`;
const items=[];
...
if(!items.length){
h+=`<div style="background:var(--surface);border:0.5px solid ${C.bt};border-radius:6px;padding:16px;text-align:center;font-size:11px;color:${C.ts};">Bu gün ders yok</div>`;
}else{
...
h+=`<div style="background:var(--bg-app);border:0.5px solid ${C.bt};border-radius:8px;padding:10px 12px;">
...
const color=ratio>=1?'#791F1F':ratio>=0.75?'#A32D2D':ratio>=0.5?'#854F0B':'var(--tp)';
...
h+=`<div style="background:var(--surface);border:0.5px solid ${C.bt};border-radius:8px;padding:10px 12px;margin-bottom:8px;">
